<?php 
    ...
    function renderGeneralProduct($productArr) {
        foreach ($productArr as $item) {
            ...
            if (!$amount > 0) {
                ...
                $soldoutLabel = '';
                $button = '
                    <button type="submit" class="btn add__btn">
                        <i class="fa-solid fa-plus"></i> Xem sau
                    </button>
                    <a href="index.php?pg=buynow&productId='.$id.'" class="btn buy__btn">
    ...
    function renderSummaryProduct ($cart) {
        $product_html = '';
        $total = 0;
        foreach ($cart as $item) {
            ...
            $subTotal = $price * $qty;
            $total += $subTotal;
            $path = "./views/assets/images/$img";
            $product_html .= '
                <div class="cart-summary__product flex-between flex-full">
                    <div class="flex g6">
                        <div class="common-banner product__banner" style="background-image: url(' . $path . ')"></div>
                        <h4 class="product__name body-text2">' . $name . '</h4>
                    </div>
                    <div class="flex-column flex-between end">
                        <h4 class="product__subtotal text">$' . $price . '</h4>
                        <h4 class="product__qty text">x' . $qty . '</h4>
                    </div>
                </div>
            ';
        }
